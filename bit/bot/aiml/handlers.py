...
def bot_speaks(evt):
    ...
    if 'bot-speaks' in subs.subscriptions:
        for subscriber in subs.subscriptions['bot-speaks']:
            subs.subscriptions['bot-speaks'][subscriber](json.dumps(dict(emit={'bot-speaks': 'Bot to %s: %s' %(evt.session_id,evt.message)})))
...
def person_speaks(evt):
    ...
    if 'person-speaks' in subs.subscriptions:
        for subscriber in subs.subscriptions['person-speaks']:
            subs.subscriptions['person-speaks'][subscriber](json.dumps(dict(emit={'person-speaks': 'Person to %s: %s' %(evt.session_id,evt.message)})))
